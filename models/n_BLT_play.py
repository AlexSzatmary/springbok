...
class Neutrophil(springbok.Cell):
    ...
    def secrete(self, L_condition):
        return np.array([0., self.s_rate]) * FRO(L_condition[0][0])
...
def setup(pde_stepper=make_pde_stepper(1000.), has_BLT=True, name='n_BLT',
    ...
    cg = springbok.RectCellGroup(
        ...
        n_neutrophil, n_t=61, s_rate=s_rate)
        ...
        for has_BLT in L_has_BLT:
            run = setup(pde_stepper=make_pde_stepper(ell), has_BLT=has_BLT,
                        name=('n_BLT-' + ('BLT' if has_BLT else 'BLT0') +
                              '-{:.2e}').format(ell), s_rate=s_rate)
            run.ell = ell
            run.has_BLT = has_BLT
            L_run.append(run)
    return L_run
            
def make_vary_s(L_s_rate=[1e1, 1e2, 1e3, 1e4], L_has_BLT=[False, True]):
    L_run = []
    for s_rate in L_s_rate:
        for has_BLT in L_has_BLT:
            run = setup(pde_stepper=make_pde_stepper(s_rate), has_BLT=has_BLT,
                        name=('n_BLT-' + ('BLT' if has_BLT else 'BLT0') +
                              '-{:03.0f}').format(s_rate), s_rate=s_rate)
            run.s_rate = s_rate
            run.has_BLT = has_BLT
            L_run.append(run)
    return L_run
