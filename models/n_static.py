...
class Neutrophil(springbok.Cell):
    ...
    def orient(self, L_conditions):
        ...
        try:
            self.theta = vonmises.rvs(kappa)
        except ValueError:
            print(self.index, L_conditions, kappa)
...
def setup(pde_stepper=make_pde_stepper(1000.), name='nsg'):
    n_neutrophil = 100
    L_cell = [Neutrophil(xy_0=xy_0, n_t=61)
              for xy_0 in 1000. * np.random.random((n_neutrophil, 2)) +
              np.array([1000., 0.])]
    
    model = springbok.Springbok(
        L_cell_group=[springbok.CellGroup(L_cell)], pde_stepper=pde_stepper,
        L_f=[lambda foo: lambda t, x, L_u, dudx, d2udx2: 
