...
#Required by Meerkat
L_variables = [L_r_L, L_phi_E, L_has_BLT, L_ell]
...
def job_name(r_L, phi_E, has_BLT, ell):
    return 'n_BLT0-vary-ell-r_L{}-phi_E{:.2f}-ell{}'.format(r_L, phi_E, ell) + 'BLT' + ('+' if has_BLT else '-')
def prototype():
    return d_runs[(1e8, 0.)]
...
def make_r_L_phi_E(r_L=None, phi_E=0., has_BLT=True, ell=1e3):
...
def setup(r_L, phi_E, has_BLT, ell):
    jn = job_name(r_L, phi_E, has_BLT, ell)
    run = make_r_L_phi_E(r_L=r_L, phi_E=phi_E, has_BLT=has_BLT, ell=ell)
    run.job_name = jn
